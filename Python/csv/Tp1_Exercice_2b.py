...
def ouvrir():
    ...
    txt=fichierSource.readline()#lecture d'une ligne
    while txt!="":
        zoneTexte.insert(str(ligne)+".0",txt)#2 paramètres : position d'insertion(ligne,colone) et la chaine
        ligne=ligne+1
...
def enregistrer():
    fichierSource= open(chemin,'w',encoding="utf-8")
